    ...
    client.on("myUserInfo",async({id})=>{
      if (id) {
        ...
        client.on("send-message-to-my-friend",(async (data)=>{
         ...
         console.log(myCurrentFriend)
          if (myCurrentFriend[0]?.socketId !== undefined) {
            ...
            await client.to(myCurrentFriend[0]?.socketId).emit("hoga","Hoga mara shara")
        ...
        client.on('typingMsg',async(data)=>{
          const myCurrentFriend = await this.onlineUsers.filter(U=>U.userId === data?.receiverId)
           await client.to(myCurrentFriend[0]?.socketId).emit("getTypingMsg",data)
        })

        client.on('openMessageWindow',async(data)=>{
          console.log(data)
          const myCurrentFriend = await this.onlineUsers.filter(U=>U.userId === data?.receiverId)
          await client.to(myCurrentFriend[0]?.socketId).emit('getOpenMessageWindow',data.status)
        })
        let ids = [];
