...
// import ww  from "@/app/global/firework/f_top"
  ...
  useEffect(() => {
    ...
    function color_hover_with_XY(e, X, Y, speed) {
      ...
      while (i != speed) {
        ...
        pointers[0].dy = (e.clientY + Y - pointers[0].y) * 1.0;
    ...
    function startGUI() {
      ...
      gui
        ...
        .onFinishChange(initFramebuffers);
      gui
        .add(config, "DENSITY_DISSIPATION", 0.9, 1.0)
...
`
    ...
    canvas.addEventListener("mousemove", (e) => {
      pointers[0].moved = pointers[0].down;
      pointers[0].dx = (e.offsetX - pointers[0].x) * 1.0;
      pointers[0].dy = (e.offsetY - pointers[0].y) * 1.0;
...
window.addEventListener('keydown', e => {
    ...
    function generateColor() {
      let c = HSVtoRGB(Math.random(), .8, .8);
      c.r *= 0.25;
      c.g *= 0.25;
      c.b *= 0.25;
      return c;
  ...
  return (
    <div className="absolute hidden md:block top-[65px] left-0 w-full h-[100vh]">
      {/* <Head>
            <script src={ww} defer>
            </script>
        </Head> */}
      <canvas
        ref={ref}
        class="banner_canvas w-full h-full"
        id="canvas_banner"
      ></canvas>
    </div>
  );
